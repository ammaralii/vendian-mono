...
export const UserAttributes = () => {
  ...
  const totalItems = useAppSelector(state => state.userAttributes.totalItems);
  ...
  return (
    <div>
            ...
            <tbody>
              {userAttributesList.map((userAttributes, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{userAttributes.version}</td>
                  <td>
                    {userAttributes.endDate ? <TextFormat type="date" value={userAttributes.endDate} format={APP_DATE_FORMAT} /> : null}
                  </td>
                  <td>
                    {userAttributes.effDate ? <TextFormat type="date" value={userAttributes.effDate} format={APP_DATE_FORMAT} /> : null}
                  </td>
                  <td>
                    {userAttributes.attribute ? (
                      <Link to={`/attributes/${userAttributes.attribute.id}`}>{userAttributes.attribute.id}</Link>
                    ) : (
                      ''
                    )}
                  </td>
                  <td>{userAttributes.user ? <Link to={`/employees/${userAttributes.user.id}`}>{userAttributes.user.id}</Link> : ''}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`/user-attributes/${userAttributes.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        tag={Link}
                        to={`/user-attributes/${userAttributes.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
      ...
      {totalItems ? (
        <div className={userAttributesList && userAttributesList.length > 0 ? '' : 'd-none'}>

    ...
    <div>
      <h2 id="addresses-heading" data-cy="AddressesHeading">
        ...
        <div className="d-flex justify-content-end">
          ...
          <Link to="/addresses/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            ...
            <tbody>
              {addressesList.map((addresses, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  <td>
                    <Button tag={Link} to={`/addresses/${addresses.id}`} color="link" size="sm">
                      {addresses.id}
                    </Button>
                  </td>
                  ...
                  <td>{addresses.createdat ? <TextFormat type="date" value={addresses.createdat} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{addresses.updatedat ? <TextFormat type="date" value={addresses.updatedat} format={APP_DATE_FORMAT} /> : null}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`/addresses/${addresses.id}`} color="info" size="sm" data-cy="entityDetailsButton">
                        <FontAwesomeIcon icon="eye" /> <span className="d-none d-md-inline">View</span>
                      </Button>
                      <Button
                        tag={Link}
                        to={`/addresses/${addresses.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        tag={Link}
                        to={`/addresses/${addresses.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"

...
export const Roles = () => {
  ...
  const totalItems = useAppSelector(state => state.roles.totalItems);
  ...
  return (
    <div>
            ...
            <tbody>
              {rolesList.map((roles, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{roles.role}</td>
                  <td>{roles.createdat ? <TextFormat type="date" value={roles.createdat} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{roles.updatedat ? <TextFormat type="date" value={roles.updatedat} format={APP_DATE_FORMAT} /> : null}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`/roles/${roles.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        tag={Link}
                        to={`/roles/${roles.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
      ...
      {totalItems ? (
        <div className={rolesList && rolesList.length > 0 ? '' : 'd-none'}>
